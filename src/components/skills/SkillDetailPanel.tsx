...
export function SkillDetailPanel({ skill, onClose, onEdit }: SkillDetailPanelProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" role="button" tabIndex={0} aria-label="Close skill details" onClick={onClose} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ' || e.key === 'Escape') onClose(); }}>
